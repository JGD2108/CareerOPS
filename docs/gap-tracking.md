...
| Medium | App authentication before deploy | Closed for private MVP | Optional single-user auth uses `APP_AUTH_ENABLED`, `APP_API_KEY`, `/api/v1/auth/login`, and an HttpOnly session cookie. The frontend no longer bundles a static app key. | For public multi-user use, replace with full OAuth/session auth. |
