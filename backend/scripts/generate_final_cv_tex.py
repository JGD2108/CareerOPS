...
def main() -> None:
    if len(sys.argv) != 2:
        raise SystemExit("Usage: python scripts/generate_final_cv_tex.py <job_id>")
    ...
    latest_plan = plans[0]
    response = httpx.post(
        f"http://127.0.0.1:8000/api/v1/cv-versions/{latest_plan['id']}/final-latex",
        timeout=30,
    )
    response.raise_for_status()
    cv_version = response.json()
    output_path = Path(cv_version["generated_file_path"])

    print(f"cv_version_id: {cv_version['id']}")
    print(f"status: {cv_version['status']}")
    print(f"final_latex: {output_path}")
